...
#new working directory for large area.
...
mydata= rbind(mydata, wildnet) 
(b=SpatialPoints(mydata))
#crop to  large extent.
...
# Analyse data 
all.locations= subset(mydata,yearnew >2000:2015, select=x:y) #2005 originally
plot(all.locations)
...
fn <-  exp(f2+f3+f4+f5+f6+f7+f8+f9+f10+f11+f12+coeff$Value[[1]])

plot(fn)
plot(fn, main= "intensity 2010 data")

ff <- s.detection
ff13 <- subset(ff,c(1))*coeff$Value[[13]] 
ff14 <- subset(ff,c(2))*coeff$Value[[14]]
fn2 <-  (ff13+ff14)+coeff$Value[[12]]   
...
plot(fn3, zlim = c(0.02,0.10), col = brewer.pal(n = 5, "RdBu"))
