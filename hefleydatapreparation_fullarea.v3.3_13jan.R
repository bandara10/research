...
set.seed(1234)
train <- sample(seq_len(nrow(ZTGLM.myFD1)), size = floor(0.20 * nrow(ZTGLM.myFD1)),replace=FALSE) # split the presence only dataset as detected and nondetected.
...
IPP.ignored=glm(presence~ AnnualMeanTemperature  + AnnualPrecipitation + dem +BDW_000_005
                +CLY_000_005+ DP_QLD_FPC20141+NTO_000_005+PTO_000_005+SAWC_000_005+habit3percent
                +lot_density
                ,family="binomial",weights=1000^(1-presence),data=IPP.data) # IPP.data2 added.
...
Detection.model=glm(presence~  distance_tertiaryandlink+lot_density
                    , family= "binomial", data=Detection.data)
...
IPP.corrected= glm(presence~AnnualMeanTemperature  + AnnualPrecipitation + dem +BDW_000_005+
                     CLY_000_005+ DP_QLD_FPC20141+NTO_000_005+PTO_000_005+SAWC_000_005+habit3percent+
                     lot_density,
                   family="binomial",weights=(1/p.det)*1000^(1-presence),data=IPP.data)
