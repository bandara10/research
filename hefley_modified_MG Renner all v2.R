...
X.des <- X.dess[,c(1,49,48,22,29,32,33)]#new variable list: 1,2,4,5,14, 17,24, 25,36,47,48,49
...
for (i in 1:length(n.quad)){
  ...
  X.des = as.matrix(cbind(poly(all.dat$awc, all.dat$clay, all.dat$distance_motorwayandlink
                               ,all.dat$distance_primaryandlink, all.dat$elev, all.dat$fpcnew
                               , all.dat$nitro,all.dat$sbd,all.dat$temp_max,all.dat$temp_min
                               ,all.dat$tpo,all.dat$twi ,degree = 2, raw = TRUE)))
...
habitat.r<- subset(myfullstack, c(1,2,4,5,14, 17,24, 25,36,47,48,49))
