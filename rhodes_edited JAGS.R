...
ydata <- as.matrix(read.csv('spp.csv'))
obs5 <- as.matrix(read.csv('cova.csv')) # need covariates #need wind fpr replicate counts. say 5 year blocks.
NSites <- nrow(ydata)
Y<- ncol(ydata)
NSteps <- obs5
dataList=list(Y=Y, NSites=NSites, NSteps=NSteps)
...
for (i in 1:4)
  {
    a[i] ~ dlnorm(amean,tau1) # shape parameter of gamma distribution is lognormally distributed.
  }
  # loop through sites
    for (i in 1:NSites)
  {
    ...
    b[i,1] <- a[trunc((Year[i,1] - 1995)/5.1) + 1]/Lambda[i,1] # Year to be a column. 
...
beta ~ dnorm(0,0.001)
"
initsList = list(amean=0,tau1=0,sig1=1, tau2=2,sig2=1, beta=0)

writeLines(modelString, con="TEMPmodel.txt")
jagsModel = jags.model( file="TEMPmodel.txt", data=dataList , inits=initsList , n.chains=5 , n.adapt=500 )
