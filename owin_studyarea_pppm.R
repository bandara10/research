...
all.locations= subset(myalldata ,yearnew >=1997|yearnew < 2013,select=X:Y) #,  REmove xy for ppp density
...
set.seed(123)
selected.locations=gridSample(all.locations, habitat.rr, n = 1)# add marks at this level.
...
# marks(dat.ppp) <- myalldata$yearnew # 
# dat.ppp <- unique(dat.ppp, rule="unmark")
# summary(dat.ppp)


######split based on year colomn 1 whihc is a factor
# dat.ppp <- ppp(x = selected.locations[,1], y = selected.locations[,2], window = dat.w)
# dat.ppp=unique(dat.ppp, rule= "unmark")
...
densit <- density.ppp(split_dat.ppp)  # ngrid = c(251,165)
plot(densit*1E06)
...
plot(rhohat(split_dat.ppp$`1997`,densit ))
#Quadrat test
...
en=envelope(dat.ppps,fun=Kest,funargs=list(correction="border"),global=TRUE)
plot(en)
...
dimyx = c(200,200)
bandwidth <- 2000   # decide 

plot(density(split(dat.ppp)), ribbon = TRUE)

dat.den <- density(split(dat.ppp), sigma = bandwidth, diggle = TRUE, dimyx = dimyx)
plot(dat.den*1E06)
...
# obtain bandwidth for each year if possible

dat.ppps=split(dat.ppp)   ####### split marks select as $
b <-  bw.diggle(dat.ppps$`1997`)
b
plot(b)
plot(b, xlim= c(400, 2000))

#######============================================================================================
d=density(dat.ppps$`1997`, sigma= bw.diggle(dat.ppp), ngrid = c(251,165))
plot(d)
d=d*1E06
d <- as(d,"SpatialGridDataFrame")
d.r=raster(d )
plot(d.r )

#######============================================================================================

den.5 <- density(dat.ppps$`1997`, sigma=1500 ,edge=TRUE,ngrid = c(251,165) )
plot(den.5)
den.5=den.5*1E06
den.5g <- as(den.5,"SpatialGridDataFrame")
den.5g.r=raster(den.5g )
plot(den.5g.r, main= "kernel smooethd map bandwidth 100m")
writeRaster(den.5g.r, "C:/Users/uqrdissa/ownCloud/Covariates_analysis/Mark_S/kernel1500.tif",overwrite=TRUE)

###adaptive density=================== vvaring kernel and badwidth taking 0.2 percent sightings


k=adaptive.density(dat.ppps$`2013`, f=0.2, nrep=10,ngrid = c(251,165))
plot(k)
k=k*1E06
k <- as(k,"SpatialGridDataFrame")
k.r=raster(k)
plot(k.r, main="adaptive kernel 2013" )

lga10.shp=readShapePoly("C:\\Users\\uqrdissa\\ownCloud\\Covariates_analysis\\Mark_S\\raster_stack\\LGA10new.shp")
plot(lga10.shp, add=TRUE)
plot(studyarea.shp, add=TRUE)
writeRaster(k.r, "C:/Users/uqrdissa/ownCloud/Covariates_analysis/Mark_S/kerneladaptive2013.tif",overwrite=TRUE)
par(mfrow=c(1,2))


#### hot spots for activity and clusters===================

l=scanLRTS(dat.ppps$`2013`, r=3*bw.ppl(dat.ppp))
plot(l, main= "likelihood ratio test statistic")

pvals <- eval.im(pchisq(l,df=1, lower.tail = FALSE))
dev.off()
plot(pvals, main="cluster analysis clusters_lLR_test 2013 Pvals")
ppp=solutionset(pvals <=.01)
plot(ppp, main= "clusters 2013", col ="red")
...
dat.pilot <- LSCV.density(dat.ppps$`1997`)
