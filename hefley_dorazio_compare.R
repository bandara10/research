...
w.files=c("s1_residential_dist.tif","distance_trunkandlink.tif", "distance_tertiaryandlink.tif")# get distance in km.
#w.files=c("hpop.tif")
dd <- stack(w.files)
...
pb=mydatasighting_cleaned[c(127,128,117)] # xy and year for full dataset.
pb <- subset(pb, yearnew == 2011, select=c(X, Y))
...
ff3 <- subset(ff,c(3))*coeff$Value[[13]]
fn2 <-  (f1+f2+f3)+coeff$Value[[10]]   
