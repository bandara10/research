...
ppmForm = ~ poly(TMP_MAX,TMP_MIN, RAIN_ANN, degree = 2) 
...
backg.env$D_URBAN = stand.D_URBAN
# To fit a Poisson point process model at a spatial resolution of 1km
ppmForm.2 = ~ poly(TMP_MAX,TMP_MIN, RAIN_ANN, degree = 2) + poly(D_MAIN_RDS, D_URBAN, degree = 2)
...
pred.biasCorrecta = predict(ppmFit.2, newdata=newEnv)
head(pred.biasCorrect)
predictions.2 <- cbind(xydatan, pred.biasCorrecta)
xy.rr.2 <- rasterFromXYZ(as.data.frame(predictions.2)[, c("X", "Y", "pred.biasCorrecta")])
plot(xy.rr.2, las=0)
# To find the resolution (in the range from 0.5 to 16 km):
scales = c(0.5, 1, 2, 4, 8, 16)
findres(scales, sp.xy = BlueMountains$eucalypt,
...
ppmForm.3 = ~ poly(TMP_MAX,TMP_MIN, RAIN_ANN, degree = 2) + poly(D_MAIN_RDS, D_URBAN, degree = 2)
...
newEnv = BlueMountains$env
newEnv$D_MAIN_RDS = min(stand.D_MAIN_RDS)
newEnv$D_URBAN = min(stand.D_URBAN)
pred.biasCorrectaa = predict(ppmFit.3, newdata=newEnv)
predictions.3 <- cbind(xydatan, pred.biasCorrectaa)
xy.rr.3 <- rasterFromXYZ(as.data.frame(predictions.3)[, c("X", "Y", "pred.biasCorrectaa")])
plot(xy.rr.3,asp=1)
# To find the resolution (in the range from 0.5 to 16 km):
scales = c(0.5, 1, 2, 4, 8, 16)
findres(scales, sp.xy = BlueMountains$eucalypt,
