...
col.type <- data.frame("varType"=sapply(bootstrap.sample, function (x) class(x)))
...
for(i in 1:nCol){
  if(col.type[i,1]=="numeric"){
    ...
    hist(bootstrap.sample[,col.type[i,2]],
         breaks=20,
         main=paste(col.type[i,2],sep=""),
         xlab=paste(col.type[i,2],sep="")
