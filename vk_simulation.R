...
####
s <- s.detection
a <- (s*-0.9204987-1.2423949)
a <- logit.prob(a)  #library(optiRum)
plot(a)
# #or
ss <- s.occupancy
aa <- exp(ss*0.4909962+9)
plot(aa)
final <- a*aa
plot(final)
###### THIS RESULTS is OKAY FOR SIMULATED DATA. upper limit 12000
#below is dataframe version. 
d <- as.data.frame(s.detection, xy=TRUE)
b <- d[c(-3)]
f <- d$w
a <- exp(f*-0.9204987-1.2423949)
pp <- cbind(b,a)
pred.ct <- rasterFromXYZ(as.data.frame(pp)[, c("x", "y", "a")])
plot(pred.ct)
######
dd <- as.data.frame(s.occupancy, xy=TRUE)
bb <- dd[c(-3)]
ff <- dd[c(-1,-2)]
colnames(ff)[1] <- "xx"
aaa <- (ff*0.4909962)+9
...
pp <- cbind(bb,aa)
pred.ctt <- rasterFromXYZ(as.data.frame(pp)[, c("x", "y", "xx")])
plot(pred.ctt)

density <- pred.ct*pred.ctt/160
plot(density)


beta0 21.7788970     0.26888612
2              x  0.4909962
