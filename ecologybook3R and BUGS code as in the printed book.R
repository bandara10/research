...
show(tits)                     # Look at data
str(tits)                      # Overview of data file

table(tits$latname)            # Available species (Latin short)

table(tits$name)               # Same in English

# Select Great tit and covariate data from 2013 and 
#   drop 4 sites not surveyed in 2013
tits <- tits[tits$latname == "PARMAJ",]                    # get great tit data
NA.sites <- which(apply(is.na(tits[,41:43]), 1, sum) == 3) # unsurveyed sites
tits <- tits[-NA.sites,]                                   # Drop them
y <- cbind(tits$count131,tits$count132, tits$count133)     # Counts
date <- cbind(tits$date131,tits$date132, tits$date133)     # Survey date
dur <- cbind(tits$dur131,tits$dur132, tits$dur133)         # Survey duration
...
umf <- unmarkedFramePCount(y = y, siteCovs=data.frame(elev=scale(tits[,"elev"]), forest=scale(tits[,"forest"]), iLength=1/tits[,"rlength"]),
...
plot.Nmix.resi(fm5, fm5NB, fm5ZIP)               # Produces Fig. 6???10
...
map.Nmix.resi(fm5)         # Map of average residuals for Poisson model
map.Nmix.resi(fm5NB)       # Map of average residuals for NB model
map.Nmix.resi(fm5ZIP)      # Map of average residuals for ZIP model
...
tits <- read.table("SwissTits_mhb_2004_2013.csv", header = T, sep = ";")
str(tits)                      # Overview of data file
table(tits$latname)            # Available spec (Latin short) (not in order !)
table(tits$name)               # Same in English

# Select 2013 counts (all species) and put into 3D array
y <- as.matrix(tits[,c("count131","count132", "count133")])
# Create same NA pattern in counts y as in survey date
y[is.na(cbind(tits$date131,tits$date132, tits$date133))] <- NA
# List of sites that were surveyed in 2013
keep <- which(apply(is.na(y[1:267,]), 1, sum) != 3) # surveyed sites

# Get covariate data (site and observational) and drop unsurveyed sites
elev <- tits[keep, "elev"]
route <- tits[keep, "rlength"]
forest <- tits[keep, "forest"]
date <- cbind(tits$date131,tits$date132, tits$date133)[keep,] # Survey date
dur <- cbind(tits$dur131,tits$dur132, tits$dur133)[keep,] # Survey duration

# Put counts (all species) into 3D array
y3D <- array(NA, dim = c(267, 3, 6)) # Define 3D array
for(i in 1:6){                 # Fill in slices
  y3D[,,i] <- y[((i-1)*267+1):(i*267),]
}
y3D <- y3D[keep,,]             # only keep sites surveyed in 2013

# 'Degrade' counts to mere detection/nondetection data
y3DRN <- y3D                   # Copy counts
y3DRN[y3DRN > 1] <- 1          # Overwrite any count >1 with 1 (for RN model)


library(unmarked)
spec.names <- c("Great tits", "Blue tits", "Coal tits", "Crested tits", "Marsh tits", "Willow tits")
...
head(cbind(predL[1:2], pred.c.hatL[1:2]), 10)
...
par(mfrow = c(2,3)   ;   traceplot(out2)
    ...
    # Bundle and summarize data set
    str( win.data <- list(Ysum = Ysum, M = nrow(Ysum), J = data$nsurvey[1:nsite], nspec = dim(Ysum)[2]) )
    ...
    # -----------------------------------------------------
    # Bundle and summarize data set
    str( win.data <- list(Ysum = Ysum, M = nrow(Ysum), J = data$nsurvey[1:nsite], nspec = dim(Ysum)[2], R = matrix(c(5,0,0,1), ncol = 2), df = 3) )
    ...
    mass <- tapply(data$body.mass, data$specid, mean) # Get mean species mass (all 158 spec)
    ...
    # Bundle and summarize data set
    str( win.data <- list(Ysum = Ysum, g = gmass, M = nrow(Ysum), J = data$nsurvey[1:nsite], nspec = dim(Ysum)[2]) )
    ...
    plot(data$coordx[1:267], data$coordy[1:267], xlab = "x coordinate", ylab = "y coordinate", cex = x*pm, asp = 1, pch = 16)
    points(data$coordx[which(pm == 1)], data$coordy[which(pm == 1)], cex = x*pm, col = "red", pch = 16)
